...
VALUES (
    ...
    (
        '45.678.912/0001-55',
        'Hostel Backpackers BR',
        'Hostel',
        'João Pereira',
        '(31) 98765-4321',
        'backpackers@hostel.com',
        'Matriz',
        'MG',
        'Belo Horizonte',
...
CREATE TABLE IF NOT EXISTS setandoTipos (
    ...
    fkTipoLog INT,
    CONSTRAINT fkSetandoParams FOREIGN KEY (fkParamsNotificacoes) REFERENCES paramsNotificacao (id),
    CONSTRAINT fkSetandoTipo FOREIGN KEY (fkTipoLog) REFERENCES tipoLog (id),
    CONSTRAINT pkComposta PRIMARY KEY (
        fkParamsNotificacoes,
        fkTipoLog
    )
);
...
VALUES (
        100,
        "Notificação Time dev",
        "Notificação sobre leitura do java",
        "https://hooks.slack.com/services/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",
        "#production-logs",
        1,
        1,
        1
...
SELECT p.id, p.isAtivo AS ativo, p.tituloNotificacao, p.descricao, p.url, p.canal, GROUP_CONCAT(t.tipo SEPARATOR ', ') AS tipos
FROM
    paramsNotificacao p
    INNER JOIN setandoTipos s ON s.fkParamsNotificacoes = p.id
    INNER JOIN tipoLog t ON t.id = s.fkTipoLog
GROUP BY
    p.id,
    p.isAtivo,
    p.tituloNotificacao,
...
WITH
    ...
    media_historica AS (
        SELECT AVG(total_mes) AS media_mes
        FROM (
                SELECT ano, SUM(
                        COALESCE(passageiros_pagos, 0)
                    ) AS total_mes
...
SELECT ROUND(ma.total / mh.media_mes, 2) AS indice_x
FROM mes_atual ma
...
WITH
    ...
    movimento_atual AS (
        SELECT
            ...
            destino_localidade,
            SUM(
                COALESCE(passageiros_pagos, 0) + COALESCE(passageiros_gratis, 0)
...
SELECT a.destino_uf, a.destino_localidade, ROUND(
        (
            a.total_passageiros - b.total_passageiros
        ) * 100.0 / NULLIF(b.total_passageiros, 0), 2
    ) AS crescimento_percentual
FROM
    movimento_atual a
    INNER JOIN movimento_anterior b ON a.destino_localidade = b.destino_localidade
WHERE
    b.total_passageiros >= 10000
...
SELECT
    ...
    a.destino_uf,
    CASE
        WHEN b.passageiros_anterior >= 10000 THEN ROUND(
            (
                a.passageiros_atual - b.passageiros_anterior
            ) * 100.0 / b.passageiros_anterior,
            2
        )
        ELSE NULL
...
FROM
    movimento_atual a
    LEFT JOIN movimento_anterior b ON a.destino_uf = b.destino_uf
    AND a.mes = b.mes
ORDER BY a.mes, crescimento_percentual DESC;
-- SELECT QUE SERÁ REALIZADO NA MODEL *TURISTAS PAGANTES X MES*
SELECT *
FROM
    vw_crescimento_estado_em_relacao_ao_mesmo_no_ano_anterior;
-- --------------------------------------------------------------------------------------------------------------------->
...
SELECT
    ...
    CASE
        WHEN b.passageiros_pagos >= 0 THEN ROUND(
            a.passageiros_pagos / NULLIF(b.passageiros_pagos, 0),
            2
        )
        ELSE NULL
    ...
    CASE
        WHEN b.passageiros_pagos >= 0 THEN ROUND(
            (
                a.passageiros_pagos - b.passageiros_pagos
            ) * 100.0 / b.passageiros_pagos,
            2
        )
        ELSE NULL
    END AS indice_crescimento,
    ROUND(
        a.passageiros_pagos * 100.0 / t.total_passageiros,
        2
    ) AS percentual_participacao
FROM
    movimento_atual a
    LEFT JOIN movimento_anterior b ON a.destino_localidade = b.destino_localidade
    CROSS JOIN total_periodo t
ORDER BY
    passageiros_pagos_ultimo_periodo DESC;
...
SELECT *
FROM
    vw_volume_de_turistas_mensal_local_especifico
WHERE
    destino_localidade = "CONFINS";
-- --------------------------------------------------------------------------------------------------------------------->
...
SELECT
    a.destino_localidade,
    CASE
        WHEN b.passageiros_pagos >= 5000 THEN ROUND(
            a.passageiros_pagos / NULLIF(b.passageiros_pagos, 0),
            2
        )
        ELSE NULL
...
SELECT *
FROM
    vw_indice_sazonalidade_local_especifico
WHERE
    destino_localidade = "GUARULHOS";
-- --------------------------------------------------------------------------------------------------------------------->
...
SELECT rv.ano, rv.mes, rv.destino_localidade, SUM(
...
ORDER BY rv.destino_localidade, rv.mes;

-- SELECT QUE SERÁ REALIZADO NA MODEL **
SELECT *
FROM
    vw_passageiros_pagos_localidade_ultimos_meses_ano
WHERE
    destino_localidade = "GUARULHOS";
-- --------------------------------------------------------------------------------------------------------------------->
...
SELECT rv.ano, rv.mes, rv.origem_localidade, rv.destino_localidade, CONCAT(
        rv.origem_localidade, ' → ', rv.destino_localidade
    ) AS rota, SUM(
        COALESCE(rv.passageiros_pagos, 0)
    ) AS fluxo
FROM registro_voo rv
...
ORDER BY rv.mes, fluxo DESC;

-- SELECT QUE SERÁ REALIZADO NA MODEL **
SELECT *
FROM vw_fluxo_rota_mes
WHERE
    destino_localidade = "SÃO PAULO"
    and origem_localidade = "RIO DE JANEIRO";
-- --------------------------------------------------------------------------------------------------------------------->
...
WITH
    ...
    ranking AS (
        ...
        WHERE
            total_passageiros >= 5000
    )
SELECT
    p.destino_localidade,
    CASE
        WHEN p.total_passageiros >= 5000 THEN CAST(r.posicao AS CHAR)
        ELSE 'N/A'
    END AS ranking_geral
FROM
    passageiros_por_destino p
    LEFT JOIN ranking r ON p.destino_localidade = r.destino_localidade
ORDER BY p.total_passageiros DESC;

-- SELECT QUE SERÁ REALIZADO NA MODEL *ranking de localidades*
SELECT *
FROM
    vw_posicao_no_ranking_localidade
WHERE
    destino_localidade = "RIO DE JANEIRO";
-- --------------------------------------------------------------------------------------------------------------------->
...
WITH
    ...
    crescimento AS (
        SELECT a.destino_localidade, ROUND(
                (
                    a.passageiros_atual - b.passageiros_anterior
                ) * 100.0 / b.passageiros_anterior, 2
            ) AS crescimento_percentual
        FROM
            movimento_atual a
            INNER JOIN movimento_anterior b ON a.destino_localidade = b.destino_localidade
        WHERE
            b.passageiros_anterior >= 1000
    ),
    ranking AS (
        SELECT destino_localidade, DENSE_RANK() OVER (
                ORDER BY crescimento_percentual DESC
            ) AS posicao
        FROM crescimento
    )
SELECT
    ...
    CASE
        WHEN r.posicao IS NOT NULL THEN CAST(r.posicao AS CHAR)
        ELSE 'N/A'
...
FROM (
    ...
    LEFT JOIN ranking r ON destinos.destino_localidade = r.destino_localidade
ORDER BY
    CASE
...
SELECT *
FROM
    vw_posicao_no_ranking_crescimento_localidade
WHERE
    destino_localidade = "JUAZEIRO DO NORTE";
-- --------------------------------------------------------------------------------------------------------------------->